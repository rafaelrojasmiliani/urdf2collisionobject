...
urdf_to_collision_object(urdf::Model &_urdf_model) {
  ...
  std::iter_swap(result.primitive_poses.begin(),
                 result.primitive_poses.begin() + root_index);
  std::iter_swap(result.primitives.begin(),
                 result.primitives.begin() + root_index);
  return result;
}
...
void update_frames(const Eigen::Isometry3d &_base_pose,
  ...
  result.primitive_poses[0] = pose;
  for (std::size_t i = 1; i < result.primitive_poses.size(); i++) {

    tf2::doTransform(result.primitive_poses[i], result.primitive_poses[i],
                     base_transform);
  }

  for (std::size_t i = 0; i < result.subframe_poses.size(); i++) {

    tf2::doTransform(result.subframe_poses[i], result.subframe_poses[i],
                     base_transform);
  }
}
} // namespace urdf_to_collision_object
